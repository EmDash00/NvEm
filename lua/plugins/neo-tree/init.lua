return {
  ...
  after = function()
    local util = require("plugins.neo-tree.util")
    ...
    require("neo-tree").setup({
      ...
      commands = {
        trash = util.trash,
        trash_visual = util.trash_visual,
        sys_open = util.sys_open,
        telescope_find = function(state)
          local node = state.tree:get_node()
          local path = node:get_id()
          require('telescope.builtin').find_files(util.get_telescope_opts(state, path))
        end,
        telescope_grep = function(state)
          local node = state.tree:get_node()
          local path = node:get_id()
          require('telescope.builtin').live_grep(util.get_telescope_opts(state, path))
        end,
      },
      window = {
        position = "left",
      ...
      document_symbols = {
        ...
        kinds = {
          ...
          Constant = { icon = completion_kinds.Constant, hl = "@constant" },
          Variable = { icon = completion_kinds.Variable, hl = "@variable" },
